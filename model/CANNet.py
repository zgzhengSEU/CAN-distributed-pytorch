...
class CANNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.backend = make_layers(
            self.backend_feat, in_channels=1024, dilation=True)
    ...
    def forward(self, x):
        ...
        # ave1=F.relu(ave1)
        s1 = F.interpolate(ave1, size=(
            fv.shape[2], fv.shape[3]), mode='bilinear', align_corners=True)
        c1 = s1-fv
        w1 = self.conv1_2(c1)
        w1 = torch.sigmoid(w1)
        # S=2
